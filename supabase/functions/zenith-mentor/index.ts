...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const formattedActivities = activities && activities.length > 0 
      ? activities.map((a: any) => `- ${a.activity_type} em ${new Date(a.created_at).toLocaleDateString('pt-BR')} (Score: ${a.score || 0})`).join('\n')
...
- Mantenha as respostas curtas e objetivas, com no máximo 2 parágrafos curtos.`;
    ...
    const reply = aiData.choices?.[0]?.message?.content || 'Desculpe, não consegui gerar uma resposta neste momento.';
