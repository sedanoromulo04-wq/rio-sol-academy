...
- **Erro de CORS**: O sistema tenta realizar requisições para `https://api.goskip.dev/v1/projects/config/public`, que são bloqueadas pela política de CORS (_Cross-Origin Resource Sharing_). Isso pode afetar configurações dinâmicas do ambiente "Skip".
...
Recomenda-se como prioridade tratar os erros do Supabase para exibir toasts/alertas de erro no login, implementar as páginas de cadastro e recuperação de senha, e configurar _guards_ nas rotas para proteger as páginas internas (redirecionando para `/login`).
